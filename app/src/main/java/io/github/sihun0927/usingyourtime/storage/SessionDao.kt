 ...
 * 재동기화가 공백이 유예 안이면 그 행을 **되살리고**(규칙 2·3), 넘으면 추정 종료 시각으로
 * 닫는다(규칙 4). 여기서 미리 닫으면 재동기화가 판정할 재료가 사라진다.
 *
...
interface SessionDao {
    ...
    /** 열린 세션 행의 마지막 생존 시각을 덮어쓴다. 세션 진행 중 1분마다 나온다(스펙 7절). */
    @Query("UPDATE sessions SET last_alive_at = :lastAliveAtMillis WHERE id = $OPEN_SESSION_ID")
