...
internal fun GracePeriodSetting(
    ...
    Column(
        ...
        Text(
            text = stringResource(R.string.setting_grace_period_label),
            style = MaterialTheme.typography.titleMedium,
        )
        Text(
            text = stringResource(R.string.setting_grace_period_description),
            style = MaterialTheme.typography.bodyMedium,
            color = MaterialTheme.colorScheme.onSurfaceVariant,
        )
        FlowRow(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            GRACE_PRESET_MINUTES.forEach { minutes ->
                FilterChip(
                    selected = minutes == graceMinutes,
                    onClick = { onGraceMinutesChange(minutes) },
